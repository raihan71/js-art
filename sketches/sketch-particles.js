...
const sketch = ({ width, height, canvas }) => {
  ...
  for (let i = 0; i < numCircles; i++) {
    ...
    for (let j = 0; j < numFit; j++) {
      ...
      colMap = colors[random.rangeFloor(0, colors.length)];
      colMap = (t) => {
        return `rgb(
          ${Math.floor(math.lerp(r, 255, t))},
          ${Math.floor(math.lerp(g, 255, t))},
          ${Math.floor(math.lerp(b, 255, t))}
        )`;
      };
...
const start = async () => {
  ...
  imgA = await loadImage('assets/images/david.jpg');
  imgB = await loadImage('assets/images/urban-jacket.jpeg');
